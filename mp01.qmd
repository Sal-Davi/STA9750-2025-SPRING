...
**Eric L. Adams**
is the current New York City mayor until 2025. Below, we examine his journey from being a borough president in FY 2014 until his ascendancy to become the Mayor of New York City in 2023. His current salary as being Mayor was $258,720 in 2024. 
```{r}
#| echo: false
#| message: false
#| warning: false
...
# NOTES TO SELF:
# **USE |> PIPELINE**
# **DO NOT USE THE THREE COLUMNS PROFESSOR DID NOT WANT**
...
kable(highest_salary_details, caption = "Job Title with the Highest Average Total Pay") |>
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
...
# kable
kable(top_employee, caption = "Highest Paid Employee Overall") |>
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)



```
In 2024, Mark Tettonis earned $1,689,518.This is $1,430,768 more than what mayor earned in the same year.
...
employee_overtime <- nyc_payroll |>
  group_by(first_name, last_name) |>
  summarize(
    total_overtime = sum(ot_hours, na.rm = TRUE),  
    job_title = first(title_description),          
    leave_status = first(leave_status_as_of_june_30),#if they still work or not
    .groups = "drop"
  )

# get top 3 employees
top_employees <- employee_overtime |>
  arrange(desc(total_overtime)) |>
  slice(1:3)

# kable
kable(top_employees, caption = "Top 3 Employees with the Most Overtime Hours") |>
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)



```
We next wanted to investigate which individuals have worked the most overtime hours. The view above produces the top 3 individuals with the most overtime hours throughout the entire dataset.  Although we could have taken the average or grouped by year, the sum was the appropiate metric here as we wanted to measure how much employees have been able to clock in the amount of overtime hours this past decade. Interestingly, Islam, Md, has been able to recieve almost 60,000 hours, and is still an active employee. That is an average of 12,058 overtime hours per year. 
...
kable(temp |> rename(
  "Employees outside of nyc" = work_location_borough,
...
#Without the following scale, the chart on the y-axis would be incomprehensible due to the large values. I needed to convert it to billions. Credit: GPT
...
2. The increase from 2014 to 2024 is substantial, reflecting long-term expansion in city expenditures. 
...
The CATS comissioners have wanted to analyze the potential upside in hiring more employees to reduce the amount of overtime used. It is theorized that the city can save money on total overtime pay with increasing the amount of regular hours work by hiring new employees.
```{r}
library(dplyr)
library(knitr)
library(kableExtra)
...
# Overtime analysis for employees with a valid hourly_rate and >0 overtime hours
...
# Display the overtime analysis as a neatly formatted table
kable(overtime_analysis, 
      caption = "Top 5 Overtime Analysis by Agency and Job Title (Unified Hourly Rate)") |>
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
...
This measure will help streamline operations and trim excess spending on headcount, positioning the city for a more sustainable future. Over a decade this will save an average of over **$250 million.**
...
1. Many employees earn more than the Mayor, our highest position in the city.
2. From 2014 to 2024, the City's payroll grew over $8 billion.
3. Many agencies rely on overtime pay, hinting at staff shortages or avenues of abuse by certain individuals.
4. A cost-control mechanism needs to be implemented.

**External Sources:**
1. Github Co-Pilot

2. GPT (debugging) For questions 5,7,8 & Policy I.
