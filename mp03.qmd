...
Below, we are pulling in the two datasets for this project straight from GitHub, teeing them up to fuel the exploration that follows.
...
```
To spotlight the tonal palette of our merged dataset, we spun up a vibrant **rose (polar bar) chart** that fans out each musical key around a full 360° circle. Every wedge represents one key it displayshow many distinct tracks are in that key, giving you an instant feel for which tonal centers dominate our playlists. Right away you can see that **C steals the crown—spiking the highest with 1,185 tracks—while neighboring keys like C♯/D♭ and D trail closely behind.** 
...
# 1. Find the artist of “Fashion Killa”
...
playlist_df <- bind_rows(
  sample_tracks(top_dance_cooccur,   3),
  sample_tracks(similar_songs,       2),
  sample_tracks(other_songs,         3),
  sample_tracks(cooccur_songs,       5)
)
...
playlist_df |>
  ...
  kable(
    caption = "Randomly Sampled Tracks for Your New Playlist",
    align   = c("l", "l")
  )
...
song_list <- c(
  ...
  "Canal St.",
  "Angels",
  "4 Minutes (feat. Justin Timberlake & Timbaland) - feat. Justin Timberlake And Timbaland",
  "Promise",
  "No Option",
  "Skyline To"
)
...
song_popularity <- inner_joined_data |>
  ...
  mutate(`Track Name` = factor(track_name, levels = song_list)) |>
  arrange(`Track Name`) |>
  select(`Track Name`, `Artist Name`, Popularity)

song_popularity |>
  kable(
    caption = "Popularity Scores for Selected Tracks",
    align   = c("l", "l", "r")
  )
