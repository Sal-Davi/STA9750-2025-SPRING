...
load_playlists <- function(n = 10) {
  ...
  # Compute the start indices for each slice
...
# Helper to take the top n rows and extract track + artist names
...
- **Audio-feature similarity** — distance from the seed set across tempo, energy, valence, and timbre  
...
- **Artist catalog depth** — standout releases that best represent each featured artist  

We also stratified selections by Spotify popularity to showcase both mainstream hits and hidden gems:

- **Popular ≥ 60** — well-known tracks with broad listener reach  (6 are included)
